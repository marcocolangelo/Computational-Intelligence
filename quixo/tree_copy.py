...
class MonteCarloTreeSearchNodeNoGreedy():
    def __init__(self, state: Board, player_id, d, id, root_player,parent : 'MonteCarloTreeSearchNodeNoGreedy' = None, parent_action=None, num_simulations=100, c_param=0.1):
        self.state : Board = state                   # The state of the board
        self.parent : MonteCarloTreeSearchNodeNoGreedy = parent                 # Parent node
        ...
        self.children: list[MonteCarloTreeSearchNodeNoGreedy] = []  # It contains the children nodes
    ...
    def expand(self):	
        ...
        child_node = MonteCarloTreeSearchNodeNoGreedy(state=next_state, player_id=p_id, d=self.depth+1, id=len(self.children)+1,root_player=self.root_player, parent=self, parent_action=action, num_simulations=self.num_simulations, c_param=self.c_param)
    ...
    def main(self):
        root = MonteCarloTreeSearchNodeNoGreedy(Board(),1, 0, 0,root_player=1)
...
if __name__ == '__main__':  
    mc = MonteCarloTreeSearchNodeNoGreedy(Board(),1, 0, 0,root_player=1)
    mc.main()
