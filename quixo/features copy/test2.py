...
if __name__ == '__main__':
    ...
    for ns in [1]:
        for cp in [0.1]:
            ...
            np.save("lost_test2.npy", losing_board)
            with open('lost_test2.txt', 'a' if os.path.isfile('lost.txt') else 'w') as file:
                        file.write("\n")
                        file.write(str(losing_board))    
                        
                        #print(losing_board)

            np.save("won_test2.npy", winning_board)
            with open('won_test2.txt', 'a' if os.path.isfile('won.txt') else 'w') as file:
                        file.write("\n")
                        file.write(str(losing_board))
                        #print(winning_board)
