...
from minimax_MB import MiniMax



class MonteCarloTreeSearchNodeMB():
    def __init__(self, state: Board, player_id, d, id, root_player,parent : 'MonteCarloTreeSearchNodeMB' = None, parent_action=None, minmax_depth = 4,num_simulations=100, c_param=0.1,duration = 1):
        self.state : Board = state                   # The state of the board
        self.parent : MonteCarloTreeSearchNodeMB = parent                 # Parent node
        ...
        self.children: list[MonteCarloTreeSearchNodeMB] = []  # It contains the children nodes
    ...
    def expand(self):	
        ...
        child_node = MonteCarloTreeSearchNodeMB(state=next_state, player_id=p_id, d=self.depth+1, id=len(self.children)+1,root_player=self.root_player, parent=self, parent_action=action, num_simulations=self.num_simulations, c_param=self.c_param,minmax_depth=self.minmax_depth)
    ...
    def backpropagate(self):
        ...
        if self.player_id == self.root_player:
            maximizing_player = True
        else:
            maximizing_player = False
        alfa = float('-inf')
        beta = float('inf')
        minimax = MiniMax(self, self.minmax_depth, maximizing_player, self.player_id, self.num_simulations, self.c_param)
        eval= minimax.minimax(self, self.minmax_depth, maximizing_player, alfa= alfa, beta =beta )
        self._results[eval] += 1.
        if self.parent:
            self.parent.backpropagate()

    def classical_backpropagate(self, result):
        self._number_of_visits += 1.
        self._results[result] += 1.
        if self.parent:
            self.parent.classical_backpropagate(result)
    ...
    def best_child(self, c_param,iter_sim = 10):
        # if iter_sim == 0.10 * self.num_simulations:
    ...
    def _tree_policy(self):
        ...
        #print(f"In tree policy ritorno il current_node : {current_node}")
        return current_node #return the last generated leaf node
    ...
    def best_action(self):
        ...
        while time.time() - start_time < self.duration:
            v = self._tree_policy()
            #print(f"In best_action -> v: {v}")
            reward = v.rollout()
            # # if i / simulation_no >= 0.15:
            # #     v.classical_backpropagate(reward)
            # else:
            # if v._results[reward] == None:
            #     v._results[reward] = 1
            # else:
            #v._results[reward] += 1.
            v.classical_backpropagate(reward)
...
#     def main(self):
#         root = MonteCarloTreeSearchNodeMB(Board(),1, 0, 0,root_player=1)
...
# if __name__ == '__main__':  
#     mc = MonteCarloTreeSearchNodeMB(Board(),1, 0, 0,root_player=1)
#     mc.main()
